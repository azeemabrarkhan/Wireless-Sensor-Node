...
def run_java_files():
    ...
    try:
        ...
        if len(ports) > 0:
            std_out = subprocess.check_output(['bash', '/home/pi/Desktop/sm_rpi_sensors/interface/rsc/run.sh', '-'.join(ports)])
        else:
    ...
    if GPIO.input(buttonPin) == 1:
        ...
        if GPIO.input(buttonPin) == 0:
            ...
            if elapsed <= 1:
                ...
                logging.info ("Test line: Minus than 5 ")
                sleep(1)
                if running_java:
                    ...
                    if check_internet():
                        ...
                        thread = Thread(target = run_java_files)#, args = (10, ))
                        thread.start()
                    else:
                        logging.info('Internet connection lost')
            elif (elapsed > 5 and elapsed <= 15):
                logging.info ("Test line: More than 5 seconds and less than 15 ")
                led_off()
                time.sleep(2)
