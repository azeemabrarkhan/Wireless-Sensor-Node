...
*/
public static IirFilterCoefficients design (String filterPassType, int filterOrder, double fcf1, double fcf2) {
   if (filterOrder < 1) {
      throw new IllegalArgumentException("Invalid filterOrder."); }
   if (fcf1 <= 0 || fcf1 >= 0.5) {
      throw new IllegalArgumentException("Invalid fcf1."); }
   if (filterPassType.equals("bandpass") || filterPassType.equals("bandstop")) {
      if (fcf2 <= 0 || fcf2 >= 0.5) {
...
private static double[] calculateACoefficients (String filterPassType, int filterOrder, double fcf1, double fcf2) {
   if(filterPassType.equals("lowpass") || filterPassType.equals("highpass"))
...
private static double[] calculateACoefficients_bp_bs (String filterPassType, int n, double f1f, double f2f) {
   ...
   double flip = (filterPassType.equals("bandstop")) ? -1 : 1;
...
private static double[] calculateBCoefficients (String filterPassType, int filterOrder, double fcf1, double fcf2) {
   if (filterPassType.equals("lowpass"))
      {
         int[] a = calculateBCoefficients_lp(filterOrder);
         return ArrayUtils.toDouble(a); 
      }
   else if (filterPassType.equals("lowpass"))
      {
         int[] a = calculateBCoefficients_hp(filterOrder);
         return ArrayUtils.toDouble(a); 
      }
   else if (filterPassType.equals("bandpass"))
      {
...
private static double calculateScalingFactor (String filterPassType, int filterOrder, double fcf1, double fcf2) {
...
private static double calculateScalingFactor_lp_hp (String filterPassType, int n, double fcf) {
   ...
   else if (filterPassType.equals("highpass"))
   {
...
private static double calculateScalingFactor_bp_bs (String filterPassType, int n, double f1f, double f2f) {
   double tt = Math.tan(2 * Math.PI * (f2f - f1f) / 2);    // tangent of theta
   double ctt_tt = (filterPassType.equals("bandpass")) ? 1 / tt : tt; // contangent or tangent of theta
